...
def calculate_linewidth(x,ppm):
    ...
    #
    #
    # !!! Altered to capture cases were left side is completely below max_peak/2
    ...
    for i in range(x.shape[0]):
        ...
        #print(left_side)
        try: left_ind = np.amin(np.where(left_side>max_peak/2))+gaba_min_ind
        except: left_ind = gaba_min_ind
        #print(left_ind)
        right_side = spec[ind_max_peak:]
        #print(right_side)
        try: right_ind = np.amax(np.where(right_side>max_peak/2))+gaba_min_ind+ind_max_peak
        except: right_ind = gaba_max_ind
